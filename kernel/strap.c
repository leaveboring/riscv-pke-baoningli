...
void smode_trap_handler(void) {
  ...
  // sprint("strap from %d, epc = 0x%lx\n", hartid, current[hartid]->trapframe->epc);
  // sprint("current = 0x%lx, read = 0x%lx\n", current[hartid]->trapframe->epc, read_csr(sepc));
  // sprint("in smode handler, tf = 0x%lx, epc = 0x%lx\n", current[hartid]->trapframe, read_csr(sepc));
