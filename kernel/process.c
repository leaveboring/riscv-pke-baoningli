...
int do_fork(process *parent) {
  ...
  for (int i = 0; i < parent->total_mapped_region; i++) {
    ...
    case CODE_SEGMENT:
      ...
      // panic( "You need to implement the code segment mapping of child in
      // lab3_1.\n" );
      {
        uint64 parent_va = parent->mapped_info[i].va;
        uint64 parent_pa = lookup_pa(parent->pagetable, parent_va);
        user_vm_map(child->pagetable, parent_va,
                    PGSIZE * parent->mapped_info[i].npages, parent_pa,
                    prot_to_type(PROT_READ | PROT_EXEC, 1));
        sprint("do_fork map code segment at pa:%lx of parent to child at "
               "va:%lx.\n",
               parent_pa, parent_va);
...
int increase_sem(int sem) {
  ...
  if (sems[sem].wait_head) {
    ...
    if (t->queue_next == 0)
      sems[sem].wait_tail = 0;
    insert_to_ready_queue(t);
  }
  return 0;
}

int decrease_sem(int sem) {
  if (!is_valid_sem(sem))
    return -1;
  sems[sem].value--;
