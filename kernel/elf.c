...
void* elf_process_alloc_mbc(process* p, uint64_t elf_pa, uint64_t elf_va, uint64_t size) {
    kassert(size < PGSIZE);
    void* pa = alloc_page();
    if (pa != NULL) {
        panic("uvmalloc mem alloc succeed\n");
    }
    memset(pa, 0, PGSIZE);
    return pa;
}

void substitute_bincode_elf(process *p, const char *path) {
  //elf loading. elf_ctx is defined in retrieve command line arguements.
    ...
    if( ph_addr.flags == (SEGMENT_READABLE|SEGMENT_EXECUTABLE) ){
      for(int j = 0; j < PGSIZE/sizeof(mapped_region); j++) {
        if(p->mapped_info[j].seg_type == CODE_SEGMENT) {
          sprint( "CODE_SEGMENT added at mapped info offset:%d\n", j );
          // free the original page and alloc new page
